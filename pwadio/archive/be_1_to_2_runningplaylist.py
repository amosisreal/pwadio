...
table_name = "pwadio_running_playlist"
cursor = conn.cursor()

cursor.execute("select Unique_ID, date_added, time_played, artist_name_text, track_name_text from " + table_name + " order by id limit 700")
...
for row in rows:
    try:
	logging.warning("Trying to set processing time to existing processing time.")
	pt = ProcessingTime.objects.get(date_added=row['date_added'])
    except:
	logging.warning("Processing time set to default.")
